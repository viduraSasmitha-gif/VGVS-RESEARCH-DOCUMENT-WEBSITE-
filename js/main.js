...
function initStats() {
    ...
    // Filter to count only Admin papers (consistent with research.html)
    const validPapers = papers.filter(p => {
        return (p.uploaderRole === 'admin') ||
            (p.author && (p.author.toLowerCase() === 'admin' || p.author.toLowerCase() === 'vidura'));
    });

    const paperCount = validPapers.length; // Show actual count (0 if empty)
